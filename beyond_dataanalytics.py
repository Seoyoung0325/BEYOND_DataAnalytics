# -*- coding: utf-8 -*-
"""BEYOND_dataAnalytics.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/github/Seoyoung0325/BEYOND_DataAnalytics/blob/main/BEYOND_dataAnalytics.ipynb
...
#plt.rc('font', family='NanumGothic')  #코랩버전 한글폰트 설정
...
# 코랩에서 직접 업로드하는 경우
"""uploaded = files.upload()   # 1~8호선 데이터(.csv) 선택
...
uploaded = files.upload()   # 9호선 데이터(.xlsx) 선택
...
def analyze_current_congestion(df_model, now=None,
    ...
    slot         : int
    """
    from zoneinfo import ZoneInfo   ########## 프론트엔드 연결 후 사용자 컴퓨터 시간 자체를 넘기는 걸로 수정
...
# 테스트 실행: 현재 혼잡 역/호선 조회 + 추천 경로에 경고 문구 반영

congested_now = print_current_congestion_analysis(df_model, top_percentile=90.0, z_threshold=1.5)

print(f"\n{'='*45}")
print("추천 경로별 실시간 혼잡 경고 체크")
print(f"{'='*45}")

for r in routes:
    warning = check_route_congestion_warning(r, congested_now)
    tag = '⭐ 추천' if r == routes[0] else ''
    print(f"\n경로 {r['route_idx']}  |  쾌적도 점수: {r['comfort_score']}  {tag}")
    if warning:
        print(f"  {warning}")
    else:
        print("  ✅ 현재 혼잡 구간과 겹치지 않습니다.")

"""## 10. 혼잡도 분석 결과 시각화 (발표자료용)"""

"""plt.rcParams['axes.unicode_minus'] = False  # 마이너스 기호 깨짐 방지 (한글 폰트 사용시 필수)

# 실제 서울 지하철 노선도 색상 (발표자료 신뢰도를 위해 실제 색상 사용)
LINE_COLORS = {
...
def get_line_color(line):
    return LINE_COLORS.get(line, '#888888')

# Percentile x Z-score 산점도 (혼잡 판정 로직/방법론 설명용)
def plot_percentile_zscore_scatter(df_model, day_type, slot,
                                   top_percentile=90.0, z_threshold=1.5, save_path=None):
    snapshot = df_model[
        (df_model['요일구분'] == day_type) &
        (df_model['time_minutes'] == slot) &
        (df_model['혼잡도'] > 0)
    ...
    snapshot = snapshot.merge(station_stats, on=['호선', '출발역', '상하구분'], how='left')
    snapshot['z_score'] = (snapshot['혼잡도'] - snapshot['평소평균']) / snapshot['평소표준편차'].replace(0, np.nan)

    is_congested = (snapshot['percentile'] >= top_percentile) | (snapshot['z_score'] >= z_threshold)

    fig, ax = plt.subplots(figsize=(9, 6))
    ax.scatter(snapshot.loc[~is_congested, 'percentile'], snapshot.loc[~is_congested, 'z_score'],
               color='#CCCCCC', s=20, label='일반')
    ax.scatter(snapshot.loc[is_congested, 'percentile'], snapshot.loc[is_congested, 'z_score'],
               color='#E6186C', s=35, label='혼잡 판정')

    ax.axvline(top_percentile, color='#0052A4', linestyle='--', linewidth=1,
               label=f'상위 {100 - top_percentile:.0f}% 기준선')
    ax.axhline(z_threshold, color='#996CAC', linestyle='--', linewidth=1,
               label=f'z-score {z_threshold} 기준선')

    ax.set_xlabel('percentile (전체 역 대비 상대 순위, %)')
    ax.set_ylabel('z-score (역 자체 평소 대비 표준편차)')
    ax.set_title('혼잡 판정 로직: Percentile × Z-score', fontsize=14, fontweight='bold')
...
# ── 실행: 발표자료용 4종 차트 생성 및 저장 ──────────────────────────────
congested, line_ranking, day_type, slot = analyze_current_congestion(df_model)
plot_percentile_zscore_scatter(df_model, day_type, slot)"""